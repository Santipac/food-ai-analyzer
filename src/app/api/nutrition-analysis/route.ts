...
import { nutritionRateLimiter, globalRateLimiter, createRateLimitResponse } from '@/lib/rate-limiter';
...
export async function POST(req: Request) {
    ...
    // Check nutrition-specific rate limit
    const nutritionCheck = nutritionRateLimiter.checkLimit(req);
    if (!nutritionCheck.allowed) {
        return createRateLimitResponse(nutritionCheck.resetTime);
    }
    ...
    try {
        ...
        try {
            ...
            response.headers.set('X-RateLimit-Remaining-Nutrition', nutritionCheck.remaining.toString());
